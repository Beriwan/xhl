...
class XhlSpiderMiddleware(object):
...
class XhlDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class MyproxisSpiderMidleware(object):
    ...
    def __init__(self, ip=''):
        self.ip = ip
        # self.r = Redis(host='192.168.0.104', port=6379, db=0)
    ...
    #
    # def get_proxy1(self):
    #     r = RedisCluster().get_proxy()
    #     return r
    #
    # def get_proxy2(self):
    #     proxies = self.r.lrange('ip_proxy', 0, -1)
    #     proxy = random.choice(proxies).decode('utf-8')
    #     return proxy

    def process_request(self, request, spider):
        # proxy = self.get_proxy1()
        proxy = '10.10.0.112:808'
